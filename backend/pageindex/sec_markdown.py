...
def build_markdown_for_company(doc: CompanySectionDoc) -> str:
    ...
    lines: List[str] = [
        ...
        f"- CIK: {doc.cik}",
        f"- Accession Number: {doc.accession_number}",
...
def materialize_markdown_docs(docs: List[CompanySectionDoc], docs_dir: Path) -> List[Dict[str, str]]:
    ...
    for doc in docs:
        ticker_part = _safe_filename_part(doc.ticker, "UNKNOWN")
        filing_date_part = _safe_filename_part(doc.filing_date, "unknown-date")
        accession_part = _safe_filename_part(doc.accession_number, "unknown-accession")
        md_path = docs_dir / f"{ticker_part}_{filing_date_part}_{accession_part}_10k_supply_chain.md"
        ...
        registry.append(
            {
                ...
                "filing_document_url": doc.filing_document_url,
