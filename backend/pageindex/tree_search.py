        ...
        trace.append(
            {
                ...
                "selected_node_ids": [n.get("node_id") for n in candidates],
                "llm_thinking": "Deterministic selection because candidate count is within the per-level cap.",
                "fallback_used": False,
            }
        )
...
def build_citation_label(
    *,
    ...
    """Build a stable citation label carried into answer generation."""
    date_part = filing_date or "unknown-date"
    title_part = title or "Untitled"
    return f"{ticker} | {date_part} | {title_part} | node {node_id} | selector {selector}"
...
Each evidence block starts with an exact citation label in square brackets.
Every substantive claim in your answer must end with one or more citation labels copied verbatim from the context.
Do not invent citations. Do not use knowledge outside the context.
Prefer concise bullet points when the question asks about disclosed risks, factors, or findings.
