...
from sanctions.matcher import normalize_name, parse_aliases

from .neo4j_store import Neo4jStore
from .topology import build_multi_tier_topology

FETCH_SANCTIONS_SQL = """
SELECT source_entity_id, primary_name, aliases, sanctions_type, sanctions_programs, date_published, nationality, citizenship, address_text, source_file_name
...
FETCH_TRADE_SQL = """
SELECT ref_year, reporter_desc, partner_desc, cmd_code, cmd_desc, flow_desc, primary_value, qty, net_wgt
FROM source_comtrade_flows
WHERE tenant_id = %(tenant_id)s
ORDER BY ref_year DESC, primary_value DESC
""".strip()

FETCH_HAZARD_SQL = """
SELECT event_id, state, cz_name, year, event_type, damage_property_usd, damage_crops_usd, begin_lat, begin_lon, end_lat, end_lon
FROM source_noaa_storm_events
WHERE tenant_id = %(tenant_id)s
ORDER BY year DESC, event_id DESC
""".strip()

FETCH_REGULATORY_SQL = """
SELECT source_record_hash, company_name, letter_issue_date, subject, issuing_office
FROM source_fda_warning_letters
WHERE tenant_id = %(tenant_id)s
ORDER BY letter_issue_date DESC
""".strip()
...
def _fetch_rows(conn, sql: str, tenant_id: str) -> list[dict[str, Any]]:
...
def sync_graph_state(
    ...
    *,
    ...
    with connect_postgres(settings) as conn:
        sanctions_rows = _fetch_rows(conn, FETCH_SANCTIONS_SQL, tenant_id)
        trade_rows = _fetch_rows(conn, FETCH_TRADE_SQL, tenant_id)
        hazard_rows = _fetch_rows(conn, FETCH_HAZARD_SQL, tenant_id)
        regulatory_rows = _fetch_rows(conn, FETCH_REGULATORY_SQL, tenant_id)
